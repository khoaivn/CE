...
{
    auto upperTail = [](Real z)
    { return std::erfc(z / std::sqrt(Real(2))) / 2; };
    Real lo = 0, hi = 1;
...
Dataset loadDataset(const std::string &path)
{
    ...
    Graph g;
    g.n = static_cast<int>(nRaw);
    g.arcs = static_cast<size_t>(mRaw);
    ...
    for (int v = 0; v < g.n; ++v)
        g.reverse[static_cast<size_t>(v)].reserve(indegree[static_cast<size_t>(v)]);
    for (auto [u, v] : edges)
        g.reverse[static_cast<size_t>(v)].push_back(u);

    std::string resourceMarker;
    long long resourceCountRaw = 0;
    if (!(in >> resourceMarker >> resourceCountRaw) ||
        resourceMarker != "VERTEX_GAUSSIAN_COSTS_V1" || resourceCountRaw != g.n)
        throw std::runtime_error(
            "Missing resource section; expected: VERTEX_GAUSSIAN_COSTS_V1 number_of_nodes");

    Resources resources;
    resources.mu.resize(static_cast<size_t>(g.n));
    resources.variance.resize(static_cast<size_t>(g.n));
    std::vector<char> resourceSeen(static_cast<size_t>(g.n), false);
    for (int i = 0; i < g.n; ++i)
    {
        long long vertexRaw = -1;
        Real mu = 0, variance = 0;
        if (!(in >> vertexRaw >> mu >> variance))
            throw std::runtime_error("Invalid resource row; expected: vertex_id mu variance");
        if (vertexRaw < 0 || vertexRaw >= g.n)
            throw std::runtime_error("Resource vertex is outside [0,n)");
        size_t vertex = static_cast<size_t>(vertexRaw);
        if (resourceSeen[vertex])
            throw std::runtime_error("Duplicate resource row for vertex " + std::to_string(vertexRaw));
        if (!std::isfinite(mu) || mu <= 0 || mu > 1 ||
            !std::isfinite(variance) || variance <= 0)
            throw std::runtime_error("Resource mu must be in (0,1] and variance must be finite and positive");
        resourceSeen[vertex] = true;
        resources.mu[vertex] = mu;
        resources.variance[vertex] = variance;
        resources.sumMu += mu;
    }
    if (!std::isfinite(resources.sumMu) || resources.sumMu <= 0)
        throw std::runtime_error("Resource sum_mu must be finite and positive");
    std::string extra;
    if (in >> extra)
        throw std::runtime_error("Extra data after the resource section");
    return {std::move(g), std::move(resources)};
}
...
RRBank makeRRBank(const Graph &g, size_t samples, Real probability, uint64_t seed)
{
    ...
    queue.reserve(static_cast<size_t>(g.n));
    for (size_t r = 0; r < samples; ++r)
    {
        uint32_t stamp = static_cast<uint32_t>(r + 1);
        if (stamp == 0)
            throw std::runtime_error("Too many RR samples for 32-bit stamps");
        queue.clear();
        int root = target(rng);
        seen[static_cast<size_t>(root)] = stamp;
        ...
        for (int v : queue)
            bank.incidence[static_cast<size_t>(v)].push_back(static_cast<uint32_t>(r));
    }
    return bank;
}
...
uint32_t evaluate(const RRBank &bank, const std::vector<int> &items)
{
    ...
    uint64_t hits = 0;
    for (uint64_t word : covered)
        hits += static_cast<uint64_t>(__builtin_popcountll(word));
...
struct RunResult
{
    ...
    size_t tangents = 0, activeStates = 0, candidateSlots = 0;
...
size_t graphStorageBytes(const Graph &graph)
{
    ...
    for (const auto &adjacency : graph.reverse)
        bytes += adjacency.capacity() * sizeof(int);
    return bytes;
}
...
size_t rrStorageBytes(const RRBank &bank)
{
    ...
    for (const auto &list : bank.incidence)
                        ...
                        const Resources &resources, Real z, Real budget)
{
    ...
    for (int e = 0; e < g.n; ++e)
    {
        ...
        if (feasible(mu, var, z, budget) && hits > best.hits)
        {
            ...
            addItem(best, e, hits, train, resources);
        }
    }
    while (true)
    {
        int chosen = -1;
        uint32_t chosenMarginal = 0;
        Real bestDensity = 0;
        for (int e = 0; e < g.n; ++e)
            if (!selected[static_cast<size_t>(e)])
            {
                ...
                if (!feasible(nextMu, nextVar, z, budget))
                    continue;
                ...
                if (density > bestDensity)
                {
                    bestDensity = density;
                    chosen = e;
                    chosenMarginal = marginal;
                }
            }
        if (chosen < 0 || chosenMarginal == 0)
            break;
...
Config parseArguments(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (a == "--graph")
            ...
            c.order = take(i);
        else if (a == "--rr-samples")
            c.rrSamples = std::stoull(take(i));
        else if (a == "--eval-samples")
            c.evalSamples = std::stoull(take(i));
        else if (a == "--p")
            c.edgeProbability = std::stold(take(i));
        else if (a == "--epsilon")
            c.epsilon = std::stold(take(i));
        else if (a == "--alpha")
            c.alpha = std::stold(take(i));
        else if (a == "--budget")
        {
            c.budget = std::stold(take(i));
            absoluteBudget = true;
        }
        else if (a == "--budget-ratio")
        {
            c.budgetRatio = std::stold(take(i));
            ratioBudget = true;
        }
        else if (a == "--rr-seed")
            c.rrSeed = std::stoull(take(i));
        else if (a == "--eval-seed")
            c.evalSeed = std::stoull(take(i));
        else if (a == "--order-seed")
            c.orderSeed = std::stoull(take(i));
        else if (a == "--print-seeds")
            c.printSeeds = true;
        else if (a == "--help")
        {
    ...
    if (absoluteBudget && ratioBudget)
        throw std::runtime_error("Choose --budget or --budget-ratio, not both");
...
int main(int argc, char **argv)
{
    try
    {
        ...
        Dataset dataset = loadDataset(c.graph);
        Graph &graph = dataset.graph;
        Resources &resources = dataset.resources;
        if (c.budgetRatio > 0)
            c.budget = c.budgetRatio * resources.sumMu;
        ...
        if (c.order == "random")
        {
            std::mt19937_64 rng(c.orderSeed);
